...
export function run(input) {
  ...
  lines.forEach((line) => {
    let isVolumeDiscountProduct = line?.volumeDiscountProduct?.value;
    if (isVolumeDiscountProduct == "false") {
      return;
    }
    if (line?.merchandise?.__typename === "ProductVariant") {
      ...
      parsedValues.forEach((item) => {
        ...
        if (
          volumeDiscountQuantity === line.quantity &&
          totalDiscountAmount > 0 &&
          isVolumeDiscountProduct == "true"
        ) {
          discounts.push({
